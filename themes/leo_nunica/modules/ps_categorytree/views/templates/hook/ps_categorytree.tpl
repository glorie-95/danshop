    ...
    {if $nodes|count}
      <ul class="category-sub-menu">
        {foreach from=$nodes item=node}
          <li data-depth="{$depth}">
            {if $depth===0}
              <a href="{$node.link}">{$node.name}<span data-id="leo-cat-{$node.id}" style="display:none" class="leo-qty leo-cat-{$node.id} badge"></span></a>
              {if $node.children}
              ...
              {/if}
            {else}
              <a class="category-sub-link" href="{$node.link}">{$node.name}<span data-id="leo-cat-{$node.id}" style="display:none" class="leo-qty leo-cat-{$node.id} badge"></span></a>
              {if $node.children}
