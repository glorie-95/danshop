	...
	<li class="cate_{$category.id_category|intval}" >
		<div class="cate_content">
			<div>
				...
				<div class="meta-cate">
					<div class="box-cate">
						<h3 class="cate-name"><a href="{$link->getCategoryLink($category.id_category, $category.link_rewrite)|escape:'html':'UTF-8'}" title="{$category.name|escape:'html':'UTF-8'}">{$category.name|escape:'html':'UTF-8'}<span data-id="leo-cat-{$category.id_category}" class="leo-qty leo-cat-{$category.id_category}" style="display:none"></span></a></h3>
					</div>
				</div>
			</div>
		</div>
	...
	<div class="block_content">
		...
		{if $formAtts.limit > 1}
			<div id="view_all_wapper_{$random|escape:'html':'UTF-8'}" class="view_all_wapper hide">
				<a class="btn btn-primary view_all" href="javascript:void(0)">{l s='See All' d='Shop.Theme.Global'}</a>
			</div> 
		{/if}
	</div>
...
{literal} 
	ap_list_functions.push(function(){
		...
		$("ul.ul-{/literal}{$random|escape:'html':'UTF-8'}, ul.ul-{$random|escape:'html':'UTF-8'} ul"{literal}).each(function(){
			...
			if(element > limit) {
				if(!$(this).closest(".widget-category_image").hasClass('hidden-view-all')){
