                    ...
                    if (!count($this->errors)) {
                        ...
                        if (!$customer_message->add()) {
                            ...
                            if (@Mail::Send(
                                ...
                                Tools::redirectAdmin(
                                    self::$currentIndex.'&id_order='.
                                    $order->id.'&vieworder&conf=11'.'&token='.$this->token
                                );
                            }
                        }
                        $this->errors[] = $this->trans(
                ...
                if (Tools::isSubmit('partialRefundProduct')
                    ...
                    foreach ($refunds as $id_order_detail => $amount_detail) {
                        ...
                        if (empty($amount_detail)) {
                            if (!Tools::getValue('TaxMethod')) {
                                ...
                                $order_detail_list[$id_order_detail]['unit_price'] = $order_detail->unit_price_tax_incl;
                            }
                    ...
                    if ($amount >= 0) {
                        if ($order->module == 'skrill') {
                            $this_module = Module::getInstanceByName($order->module);
                            $refId = '';
                            $params = array(
                                'id_order' => $order->id,
                                'amount' => $amount
                            ...
                            $refundStatus = $this_module->refundOrder(
                                $params,
                                $refId,
                                false,
                                true
            ...
            if ($this->access('delete')) {
                if (!Tools::isSubmit('id_order_detail') && !Tools::isSubmit('id_customization')) {
                    ...
                    if ($productList || $customizationList) {
                        ...
                        // E-mail params
                        if ((Tools::isSubmit('generateCreditSlip') || Tools::isSubmit('generateDiscount'))
                            && !count($this->errors)
                        ) {
                            $customer = new Customer((int) ($order->id_customer));
                            $params['{lastname}'] = $customer->lastname;
                        ...
                        if (Tools::isSubmit('generateDiscount') && !count($this->errors)) {
                            ...
                            $cartrule->description = $this->trans(
                            ...
                            if (!$cartrule->add()) {
                                ...
                                // Update the voucher code and name
                                foreach ($language_ids as $id_lang) {
                                    $cartrule->name[$id_lang] =
                                    'V'.(int)($cartrule->id).'C'.(int)($order->id_customer).'O'.$order->id;
                                }
                                $cartrule->code =
                                'V'.(int)($cartrule->id).'C'.(int)($order->id_customer).'O'.$order->id;
                                if (!$cartrule->update()) {
                    ...
                    if (!count($this->errors)) {
                        Tools::redirectAdmin(
                            self::$currentIndex . '&id_order=' .
                            $order->id . '&vieworder&conf=31&token=' . $this->token
                        );
                    }
                }
            } else {
            ...
            if ($this->access('edit')) {
                ...
                if (!Validate::isLoadedObject($order_state)) {
                    ...
                    if ($history->sendEmail($order, $templateVars)) {
                        Tools::redirectAdmin(
                            self::$currentIndex.'&id_order='.$order->id.'&vieworder&conf=10&token='.$this->token
                        );
                    } else {
                        $this->errors[] = $this->trans(
